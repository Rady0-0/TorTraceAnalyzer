...
def extract_valid_ips(content):
    ...
    return list(set([
        ip for ip in raw_ips if is_valid_public_ip(ip)
    ]))


# ============================================
# 🔥 3. FORENSIC REPORT FILTER
# ============================================
def is_forensic_report(content):
    report_keywords = [
        "autopsy", "artifact", "file path", "metadata",
        "standard information", "attribute id", "img_",
        "vol_", "partition", "analysis report"
    ]
...
def check_network(file_data):
    ...
    # REMOVE LOW QUALITY IPS
    filtered_ips = [
        ip for ip in valid_ips
